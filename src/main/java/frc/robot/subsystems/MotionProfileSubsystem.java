 ...
 * 
 * The only routines we call on WPI_TalonSRX are....
 * 
 ...
 * [1] Calling pushMotionProfileTrajectory() continuously while the WPI_TalonSRX executes the motion profile, thereby keeping it going indefinitely.
...
// import frc.li.MotionProfileReporter;
import frc.lib.MotionProfileReporter;
...
public class MotionProfileSubsystem extends Subsystem{
	...
	/**
	 * The status of the motion profile executer and buffer inside the WPI_TalonSRX.
	...
	/**
	 * reference to the WPI_TalonSRX we plan on manipulating. We will not changeMode()
	 ...
	 * WPI_TalonSRX before we fire it.
	 */
	...
	/**
	 * Lets create a periodic task to funnel our trajectory points into our WPI_TalonSRX.
	 ...
	 * 
	 * @param WPI_TalonSRX
	 *            reference to WPI_TalonSRX object to fetch motion profile status from.
	 * @param points 
	 ...
	 * disabled and when WPI_TalonSRX is not in MP control mode.
	 */
	public void reset() {
		/*
	...
	public void control() {
		...
		if (_loopTimeout < 0) {
			...
			if (_loopTimeout == 0) {
				/*
				 * something is wrong. WPI_TalonSRX is not present, unplugged, breaker
				 * tripped
				 */
		...
		if (_talon.getControlMode() != ControlMode.MotionProfile) {
			...
			_state = 0;
			_loopTimeout = -1;
			System.out.println("WPI_TalonSRX control mode was not MotionProfile");
		} else {
			/*
			...
			switch (_state) {
				case 0: /* wait for application to tell us to start an MP */
					...
					break;
				case 1: /*
						 * wait for MP to stream to WPI_TalonSRX, really just the first few
						 * points
						 */
					/* do we have a minimum numberof points in WPI_TalonSRX */
				...
				case 2: /* check the status of the MP */
					/*
					 * if WPI_TalonSRX is reporting things are good, keep adding to our
					 * timeout. Really this is so that you can unplug your WPI_TalonSRX in
	...
	private void startFilling() {
		/* since this example only has one WPI_TalonSRX, just update that one */
	...
	/**
	 * Called by application to signal WPI_TalonSRX to start the buffered MP (when it's
	 * able to).
	 */
	...
	/**
	 * 
	 * @return the output value to pass to WPI_TalonSRX's set() routine. 0 for disable
